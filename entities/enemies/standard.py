...
from entities.combat.projectiles import Bullet, SniperBullet
...
class Sniper(Enemy):
    ...
    def shoot(self, bullet_group, target_x, target_y):
        """Dispara no frame 2 da animação de tiro com mira vetorial (diagonal)."""
        if self.shoot_cooldown == 0 and self.action == 2 and self.frame_index == 2:
            self.shoot_cooldown = 60
            spawn_x = self.rect.centerx + (self.rect.width * 0.5 * self.direction)
            ...
            if distancia > 0:
                vel_x = (dx / distancia) * 12
                vel_y = (dy / distancia) * 12
            else:
                vel_x = self.direction * 12
                vel_y = 0

            bullet = SniperBullet(spawn_x, spawn_y, vel_x, vel_y)
    ...
    def ai(self, player, screen_scroll: int, obstacle_list: list, water_group, bullet_group):
        ...
        if self.alive and player.alive:
            ...
            if self.vision.colliderect(player.rect):
                ...
                # Aplicar gravidade mesmo enquanto mira (evita flutuar)
                self.move(False, False, obstacle_list, water_group)
